...
function Producto (codigo, nombre, precio, imagen, cant, prov) {
this.codigo = codigo,
this.nombre = nombre,
this.precio = precio,
this.imagen = imagen,
...
function addProductoArray(){
    ...
    var prov = document.getElementById("idProv").value;
   
    var producto = new Producto(codigo, nombre, precio, imagen, cant, prov);
...
function LlenarTabla(){
    ...
    for (let index = 0; index < productos.length; index++) {
        ...
        scriptTabla += "<td>" + productos[index].prov + "</td>" ;
        scriptTabla += "</tr>";
    }
...
function validarCampos(){
    ...
    if (document.getElementById("idProv").value == ""){
        alert("El campo PROVIDAD no debe quedar vacío");
        return false;
    }
}
...
//function LlenadoTabla(){
    ...
    scriptTabla +=  ' <td><label for="idCantidad" >' + "Cantidad:" +'</label>';
    ...
    scriptTabla += '</td>';
    scriptTabla += '<td><img src="'+ productos[index].imagen + '"/></td>';
    scriptTabla += '<td>' + productos[index].cant + '</br>';
    scriptTabla += '<td>' + productos[index].prov + '</br>';
    scriptTabla += '<td><button type="button" class="btn btn-success" onclick="addCarrito('+productos[index].codigo+')">Agregar al carrito</button>';
    scriptTabla += '</td>';
